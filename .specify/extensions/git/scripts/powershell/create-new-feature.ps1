...
if ($Help) {
    ...
    Write-Host "  -Prefix <type>      Git Flow type prefix (e.g. feature, fix, hotfix, chore). Default: feature. Use '' to disable."
    Write-Host "  -Number N           Specify branch number manually (overrides auto-detection)"
    Write-Host "  -Timestamp          Use timestamp prefix (YYYYMMDD-HHMMSS) instead of sequential numbering"
    Write-Host "  -Help               Show this help message"
    Write-Host ""
    Write-Host "Environment variables:"
    Write-Host "  GIT_BRANCH_NAME     Use this branch name; the type prefix is still applied unless the value already contains '/'"
    Write-Host ""
    exit 0
}
...
if ($env:GIT_BRANCH_NAME) {
    ...
    $nameForNum = $branchName -replace '^[^/]+/', ''
    if ($nameForNum -match '^(\d{8}-\d{6})-') {
        $featureNum = $matches[1]
    } elseif ($nameForNum -match '^(\d+)-') {
        $featureNum = $matches[1]
    } else {
...
if (-not [string]::IsNullOrWhiteSpace($Prefix)) {
    ...
    if ($cleanPrefix -and ($branchName -notlike '*/*')) {
        $prefixSegment = "$cleanPrefix/"
        $branchName = "$prefixSegment$branchName"
    }
}

$maxBranchLength = 244
if ($branchName.Length -gt $maxBranchLength) {
    $prefixLength = $prefixSegment.Length + $featureNum.Length + 1
    ...
    $branchName = "$prefixSegment$featureNum-$truncatedSuffix"
