...
while [ $i -le $# ]; do
    ...
    case "$arg" in
        ...
        --help|-h)
            ...
            echo "  --prefix <type>     Git Flow type prefix (e.g. feature, fix, hotfix, chore). Default: feature. Use '' to disable."
            echo "  --number N          Specify branch number manually (overrides auto-detection)"
            echo "  --timestamp         Use timestamp prefix (YYYYMMDD-HHMMSS) instead of sequential numbering"
            echo "  --help, -h          Show this help message"
            echo ""
            echo "Environment variables:"
            echo "  GIT_BRANCH_NAME     Use this branch name; the type prefix is still applied unless the value already contains '/'"
            echo ""
            echo "Examples:"
...
if [ -n "${GIT_BRANCH_NAME:-}" ]; then
    ...
    # Ignore an optional leading Git Flow type segment (e.g. "feature/") when matching.
    # Check timestamp pattern first (YYYYMMDD-HHMMSS-) since it also matches the simpler ^[0-9]+ pattern
    NAME_FOR_NUM=$(echo "$BRANCH_NAME" | sed -E 's#^[^/]+/##')
    if echo "$NAME_FOR_NUM" | grep -Eq '^[0-9]{8}-[0-9]{6}-'; then
        FEATURE_NUM=$(echo "$NAME_FOR_NUM" | grep -Eo '^[0-9]{8}-[0-9]{6}')
        BRANCH_SUFFIX="${NAME_FOR_NUM#${FEATURE_NUM}-}"
    elif echo "$NAME_FOR_NUM" | grep -Eq '^[0-9]+-'; then
        FEATURE_NUM=$(echo "$NAME_FOR_NUM" | grep -Eo '^[0-9]+')
        BRANCH_SUFFIX="${NAME_FOR_NUM#${FEATURE_NUM}-}"
    else
...
# Apply the Git Flow type prefix (e.g. "feature/") to the branch name.
# Skipped when the name already carries a type segment (contains '/') so an
# explicit "tipo/id-assunto" via GIT_BRANCH_NAME is honored verbatim.
PREFIX_SEGMENT=""
CLEAN_PREFIX=$(echo "${PREFIX:-}" | sed -E 's#^/+|/+$##g; s/^[[:space:]]+|[[:space:]]+$//g')
if [ -n "$CLEAN_PREFIX" ] && [[ "$BRANCH_NAME" != */* ]]; then
    PREFIX_SEGMENT="${CLEAN_PREFIX}/"
    BRANCH_NAME="${PREFIX_SEGMENT}${BRANCH_NAME}"
fi
...
if [ -n "${GIT_BRANCH_NAME:-}" ] && [ "$BRANCH_BYTE_LEN" -gt $MAX_BRANCH_LENGTH ]; then
    ...
    PREFIX_LENGTH=$(( ${#PREFIX_SEGMENT} + ${#FEATURE_NUM} + 1 ))
    ...
    BRANCH_NAME="${PREFIX_SEGMENT}${FEATURE_NUM}-${TRUNCATED_SUFFIX}"
